<?php
    if(isset($_POST['email']) || isset($_POST['password'])) {
        ...
        require('../connect.php');
            ...
            $sql2 = "SELECT `admin_id` FROM `admins` WHERE `email`='$email' LIMIT 0,1";
            $result2 = mysqli_query($conn,$sql2);
             ...
             if (mysqli_num_rows($result2)>0) {
                ...
                $sql0 = "SELECT `attempt_no`, TIMESTAMPDIFF(MINUTE,`last_attempt`,NOW()) as timedif FROM `login_attempts_log` WHERE `admin_id`=" . $row2['admin_id'] . " ORDER BY `admin_id` DESC LIMIT 0,1";
            ...
            $sql = "SELECT `admin_id` FROM `admins` WHERE `email`='$email' AND `password`='$password' LIMIT 0,1";
            ...
            if(mysqli_num_rows($result)==0) {
                //password or email was wrong
                    ...
                    if(mysqli_num_rows($result0)>0) {
                        ...
                        if($row0['timedif']<30) {
                            ...
                            if($row0['attempt_no']<4) {
                                ...
                                $sql5 = "UPDATE `login_attempts_log` SET `attempt_no`=$temp, `last_attempt`=NOW() WHERE `admin_id`=" . $row2['admin_id'];
                            ...
                            $sql6 = "UPDATE `login_attempts_log` SET `attempt_no`=0, `last_attempt`=NOW() WHERE `admin_id`=" . $row2['admin_id'] . " ORDER BY `admin_id` DESC LIMIT 0,1";
                    ...
                    //if there hasn't been a login attempt before, store this one
                        ...
                        $sql4 = "INSERT INTO `login_attempts_log`(`admin_id`, `last_attempt`, `attempt_no`) VALUES (" . $row2['admin_id'] . ",NOW(),1)";
